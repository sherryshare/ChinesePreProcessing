...
//Delete the Chinese and Western blank in one line.
{
    ...
    while(!lineStr.empty()) {
        string ch = lineStr.substr(0,1);
	// When this char is a Western Character
        if (ch[0]>=0) {
	    //Delete the blank,tab,enter,wrap character and store the others
            if (ch[0]!=32 && ch[0]!=9 && ch[0]!=13 && ch[0]!=10)                
                retStr=retStr.insert(retStr.length(),1,ch[0]);//add char to the end of the string
        ...
        // When this char is a Chinese Character
            ...
            {
                ch = lineStr.substr(0,3);//Get the first 3 words.
            }
            else {// If the remains are less than 2 words, ignore them.
                break;
            }
            if(ch[1]<0 && ch[2]<0)//Complete Chinese word
            {
...
{   // Sentence Segmentation function of single file.       
    ...
    while(inFile.getline(buf,MAXLINE)) {// Read each line in the file.
        ...
        line=surplus+buf; // Add the remain part of the last line to the present line.
