...
namespace ctpp {
using namespace std;
RawFileProcessing::RawFileProcessing(string fileName, string outDir, bool isRawFile)
    :FileProcessing(fileName,outDir),isRawFile(isRawFile)
{
}
RawFileProcessing::~RawFileProcessing()
{
  
}

void RawFileProcessing::GetRawFormatFromAnnotatedFile(void )
{
    ChangeFileName("-raw");
    openIOFile();
    /* TO DO SOMETHING */
...
string RawFileProcessing::ChangeBracket(string lineStr)
...
char RawFileProcessing::ChangeFullCaseToHalf(string fullCaseStr)
...
string RawFileProcessing::RemoveSpaceOneLine(string lineStr)
...
int RawFileProcessing::GetUTF8BytesNum(char utf8Char)
{
    if(utf8Char >= 0)
        return 1;
...
bool RawFileProcessing::JudgeUTF8NextChar(char utf8Char)
...
int RawFileProcessing::GetSentence(string& lineStr)
...
void RawFileProcessing::SentenceSegementation(void )
